...
def optimize_model(policy_net, target_net, optimizer, memory, params: argparse.Namespace):
    ...
    state_action_values = policy_net(state_batch) # type: torch.Tensor
    print("state_action_values", state_action_values)
    state_action_values = state_action_values.gather(1, action_batch.long()) # This is from the example project. Here it makes a mess. Is it okay to just remove?
    print("state_action_values chosen", state_action_values)
    #rint("shape(policy_net(state_batch)) = ", policy_net(state_batch).shape)
